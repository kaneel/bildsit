      ...
      .replace(regPartial, (_, match) => {
        ...
        switch(operation) {
          case 'map':  {
            ...
            return Object.values(d).map(d => t(d, styles, scripts)).join('\n')
          } 
          case 'inc': {
...
function readJSTargets(content) {
  try {
    if (!!content.target && !targets.scripts[content.target]) {
      content.content = readFileSync(path.resolve(FROM_DIR, content.target).toString().trim())
